...
tuple<int, string> info, info2; // 临时存储变量信息
unordered_map<string, vector<Token>> argsMap; // 记录函数参数列表
int len; // 参数/表达式长度
int sig, tempSig; //跳转点编号
stack<int> s; // 记录循环的跳转点
int curEn, curEx; // 当前的进入与退出点
bool funCall = false;
// string temp;
...
void defFunc(vector<Token>& tokens); // 分割为单个函数逐个处理
void generateCode(vector<Token>& tokens); // 生成代码
pair<vector<Token>, vector<vector<Token>>> mergeFunc(vector<Token> expr); // 将有返回值的函数看为一个整体
void FunctionCallAsm(Token func, vector<Token> args); // 生成函数的汇编
...
vector<Token> lexicalAnalysis(string& inputCode) {
    ...
    // 定义用于匹配各种词法单元的正则表达式
    regex tokenPatterns(
            "\\b(println_int|int|return|main|void|if|else|while|continue|break)\\b"   // 匹配关键字
...
//                }
        ...
        // 跳转点可以用栈s来控制
            ...
            int leftNum = 0, rightNum = 0; // 记录左右括号的数量
            bool existElse = false; // 是否存在else
            vector<Token> sentence; // 存储要处理的部分
            i++; //移到if后面一个token
            // 处理if条件
            if (tokens[i].type == "separator" && tokens[i].value == "("){
                i++; //移到（后面一个token
                while(tokens[i].type != "separator" || tokens[i].value != ")"){
                    sentence.push_back(tokens[i]); // 把条件存入sentence
                    i++;
                }
            }
            i++; //后移一个
            calExpression(sentence); //计算表达式
            cout << "cmp eax, 0" << endl;
            cout << "je jump" << sig << endl; // 不满足条件跳到 jump_sig
            sentence.clear(); //计算完毕，清空sentence
            s.push(sig); // sig压栈（sig记录的是跳转点的序号）
            sig++;
            ...
            if (tokens[i].type == "separator" && tokens[i].value == "{") {
                i++;
                leftNum++;
                // 将所有语句存入sentence中后续处理
                // 只有当左右括号数量相等时才停止存入（考虑if内部嵌套if等的情况）
                while(leftNum != rightNum){
                    //while (tokens[i].type != "separator" || tokens[i].value != "}") {
                    if(tokens[i].value == "{"){
                        leftNum++;
...
//                cout << "Type: " << token.type << ", Value: \"" << token.value << "\"" << endl;
//            }
            generateCode(sentence); // 递归调用
            sentence.clear(); // 计算完毕，清空sentence

            if(tokens[i + 1].type == "keyword" && tokens[i + 1].value == "else"){
                existElse = true; // 后面一个是else的话
            }

            if(existElse){
                // 有else
                tempSig = s.top(); // 暂存之前的跳转点
                s.pop();
                s.push(sig); //新的跳转点
                cout << "jmp jump" << sig << endl;
                cout << "jump" << tempSig << ":" << endl;
                i += 2;
                if (tokens[i].type == "separator" && tokens[i].value == "{") {
                    i++;
                    leftNum++;
            ...
            calExpression(sentence);
            sig++;
            i++;
            s.push(sig);
            cout << "cmp eax, 0" << endl;
            cout << "je jump" << sig << endl; // 不满足条件跳到出口
            curEx = sig; // 初始化出口
            sentence.clear();
            sig++;
            ...
            curEn = s.top();
            s.pop();
            cout << "jmp jump" << curEn << endl; // 回到开始
            cout << "jump" << curEx << ":" << endl; // 离开循环
        }
            // 处理 continue
        else if(tokens[i].type == "keyword" && tokens[i].value == "continue"){
            cout << "jmp jump" << curEn << endl; // 回到开始
        }
            // 处理 break
        else if(tokens[i].type == "keyword" && tokens[i].value == "break"){
            cout << "jmp jump" << curEx << endl; // 离开循环
        }
    }
}
